...
class JobManager:
    ...
    def __init__(self, db_session, job, job_id_for_error=None):
        """
        ...
        Args:
            ...
            job_id_for_error: Job ID captured before transaction (for error handling if job becomes detached)
        """
        self.db = db_session
        self.job = job
        self.job_id_for_error = job_id_for_error if job_id_for_error is not None else getattr(job, 'id', None)
    ...
    def fail_job(self, error_message: str):
        ...
        """
        try:
            job_id = self.job_id_for_error or self.job.id
            self.logger.error(f"Failing job {job_id} with error: {error_message}")
            self.job.status = 'failed'
            self.job.error_message = error_message
            self.job.updated_at = datetime.utcnow()
            return True
        except Exception as e:
            self.logger.error(f"Failed to fail job {self.job.id}: {e}")
            return False
